/**
 * A class that handles checking answers and also tests them
 * @author Bijay Koirala with help from Brad
 *
 */
...
public class PegsControler {
	...
	public static String checkAnswer(String bAnswerBall, String userBall) {
		...
		// if both of them contain the same balls, replace both of them and add
		// one to the number of black counts
		...
		// System.out.println("Real answer after removing the blacks"
		// +sbAnswerPhase1);
		// System.out.println("User answer after removing the blacks "
		// +sbUserAnswerPhase1);

		// if each color of user's answer is in the real answer, add white count
		...
		for (int i = 0; i < sbUserAnswerPhase1.length(); i++) {
			...
			for (int j = 0; j < sbAnswerPhase1.length(); j++) {
				...
				// System.out.println("Comparing "+pegUnderConsideration+" with
				// "+ comparisionPeg+" Answer is : "+sbAnswerPhase1+" User gave:
				// "+sbUserAnswerPhase1);
				if (pegUnderConsideration == comparisionPeg) { // meaning it was
																// found
					numberOfWhites++;
					sbAnswerPhase1.setCharAt(j, 'A');
					break; // break out if you already found an answer, no need
